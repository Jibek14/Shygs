...
class Fruit:
    def __init__(self, index, is_import, title):
        self.id = index
        self.title = title
        ...
        self.owner =input(f"Введите хозяина {self.title}\t").capitalize()
        if self.title.lower() != 'импорт':
            if self.is_import == 0:
                self.tare = get_valid_float(f"Введите тару для {self.title}\t")
            else:
                self.imp_weight = get_valid_float("Введи вес за 1 у.е.\t")
